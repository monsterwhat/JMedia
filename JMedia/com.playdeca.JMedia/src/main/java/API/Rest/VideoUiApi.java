...
public class VideoUiApi {
    ...
    public String getCinemaHomeFragment() {
        try {
            ...
            // Data workflow — DB reads, continue-watching, seriesMap dedupe, series
            // enrichment, seriesCache proxy reads, v.genres / series.genres genre maps,
            // and hero-item building — runs inside ONE transaction in
...
@QueryParam("sortDirection") @DefaultValue("desc") String sortDirection,
    ...
    public String getPlaybackFragment(
        ...
        return tmpl
                .data("item", data.item)
                .data("resumeTime", data.resumeTime)
                .data("needsTranscoding", data.needsTranscoding)
                .data("needsConversion", data.needsConversion)
                .data("conversionJobId", data.conversionJobId)
                .data("conversionStatus", data.conversionStatus)
                .data("nextEpisodeId", data.nextEpisodeId)
                .data("prevEpisodeId", data.prevEpisodeId)
                .data("autoSkipIntro", data.autoSkipIntro)
    ...
    private String createSimpleCardHTML(Models.Video.Video item) {
        ...
        return String.format(
            ...
            dataAttrs, item.id, vThumb, item.id, escapeHtml(title), meta
        );
    }
